-- SeeUTrending Admin Features Database Setup
...
-- Contest submission status enum  
...
CREATE INDEX idx_contests_start_date ON public.contests(start_date);
CREATE INDEX idx_contests_end_date ON public.contests(end_date);
CREATE INDEX idx_contest_submissions_contest_id ON public.contest_submissions(contest_id);
CREATE INDEX idx_contest_submissions_user_id ON public.contest_submissions(user_id);
CREATE INDEX idx_contest_submissions_status ON public.contest_submissions(status);
CREATE INDEX idx_contest_submissions_final_score ON public.contest_submissions(final_score DESC);
CREATE INDEX idx_notifications_published ON public.notifications(published);
CREATE INDEX idx_user_notifications_user_id ON public.user_notifications(user_id);
CREATE INDEX idx_tiktok_accounts_user_id ON public.tiktok_accounts(user_id);
CREATE INDEX idx_admin_logs_admin_id ON public.admin_logs(admin_id);
CREATE INDEX idx_admin_logs_created_at ON public.admin_logs(created_at DESC);

-- Insert initial content blocks for landing page
INSERT INTO public.content_blocks (section, title, content, priority, visible) VALUES
('updates', '1000 pierwszych użytkowników otrzyma bonusowe punkty XP', 'Pierwsze 1000 użytkowników, którzy założą konto na SeeUTrending otrzyma bonusowe punkty które zwiększają szansę na zdobycie niektórych odznak i nagród, takich jak wejścia na imprezy.', 1, true),
('updates', 'Pierwszy konkurs na żywo już wkrótce!', 'Przygotowujemy pierwszy konkurs z prawdziwymi nagrodami i możliwością zdobycia prestiżowych odznak. Szykuj swoją kreatywność!', 2, true);

COMMENT ON TABLE public.contests IS 'Contest management with TikTok integration';
COMMENT ON TABLE public.contest_submissions IS 'User video submissions for contests';
COMMENT ON TABLE public.notifications IS 'Platform-wide notification system';
COMMENT ON TABLE public.content_blocks IS 'Dynamic content management for landing page';
COMMENT ON TABLE public.tiktok_accounts IS 'TikTok OAuth and account data';
COMMENT ON TABLE public.admin_logs IS 'Admin action audit trail';